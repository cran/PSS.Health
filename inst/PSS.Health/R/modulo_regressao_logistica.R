        ...
        if (estimar_ou_var_binaria()) {
          ...
          tagList(
            ...
            numericInput( ns("logistic_rate_mean"),
                          ...
                          max  = 100,
                          step = 1
            ) %>% .help_buttom(body = paste0("O percentual de eventos esperados na média da variável preditora contínua.", txt_ajuda()$txt_definido_pesquisador_OU_literatura))
          )
        }
      })
      ...
      output$balanceamentoUi <- renderUI({
        ...
        if (estimar_ou_var_binaria() & tipo != "poder") {
          ...
          numericInput( ns("balanceamento"),
                        ...
                        max   = Inf,
                        step  = .5
          ) %>% .help_buttom(txt_balanceamento_f(nome_grupo_controle(), nome_grupo_tratamento()),
